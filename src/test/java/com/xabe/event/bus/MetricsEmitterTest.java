...
public class MetricsEmitterTest {
  ...
  public void setUp() throws Exception {
    ...
    this.metricsEmitter = new MetricsEmitter(this.metricsEventPublisher);
  }
  ...
  public void shouldSentEventWhenInvokeEmitRequestEvent() throws Exception {
    ...
    when(this.metricsEventPublisher.publish(any(RequestEvent.class))).thenReturn(CompletableFuture.completedFuture(null));
  ...
  public void shouldGetCompletableFutureWithException() throws Exception {
    //Given
    final Request request = Request.of("name", LocalDateTime.now());
    when(this.metricsEventPublisher.publish(any(RequestEvent.class))).thenReturn(CompletableFuture.failedFuture(new RuntimeException()));
