...
public class ConfigOrikaTest {

  @BeforeAll
  public static void setUp() throws Exception {
    System.setProperty("property.override", "override");
    System.setProperty("dir.config", "/tmp/override");
    final String path = ConfigOrikaTest.class.getClassLoader().getResource("application.conf").getPath();
  ...
  public void shouldLoadDefaultFileApplicationConf() throws Exception {
    ...
    final Properties properties = this.getProperties(config.getConfig("pool.default"));
  ...
  public void shouldLoadFileProperties() throws Exception {
    //Given
    final String path = ConfigOrikaTest.class.getClassLoader().getResource("version.properties").getPath();
    final String decodePath = URLDecoder.decode(path, StandardCharsets.UTF_8.name());
  ...
  public void shouldLoadFilePropertiesOverride() throws Exception {
    //Given
    final String path = ConfigOrikaTest.class.getClassLoader().getResource("default.properties").getPath();
    final String decodePath = URLDecoder.decode(path, StandardCharsets.UTF_8.name());
  ...
  private Properties getProperties(final com.typesafe.config.Config config) {
